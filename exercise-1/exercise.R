...
ggplot(data = diamonds_sample) +
  geom_point(mapping = aes(x = carat, y = price), colour = "blue")
...
ggplot(data = diamonds_sample) +
  geom_point(mapping = aes(x = carat, y = price, size = price, colour = price))

# Draw a line plot (with line geometry) for `diamonds_sample`. The x-position 
# should be mapped to carat, y-position to price, and color to cut.
ggplot(data = diamonds_sample) +
  geom_line(mapping = aes(x = carat, y = price, colour = cut))

# That's kind of messy. Try using `smooth` geometry instead.
ggplot(data = diamonds_sample) +
  geom_smooth(mapping = aes(x = carat, y = price, colour = cut))
...
ggplot(data = diamonds_sample) +
  ...
  geom_smooth(mapping = aes(x = carat, y = price), alpha = 0.3, se = FALSE)
...
clarity_summary <- diamonds_sample %>% group_by(clarity) %>%
